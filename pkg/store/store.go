...
func (t *Store) ZAdd(zaddOpts schema.ZAddOptions, options ...WriteOption) (index *schema.Index, err error) {
	...
	referenceValue, ik, err := t.getSortedSetKeyVal(&zaddOpts, false)
	if err != nil {
		return nil, err
	}
...
func (t *Store) getSortedSetKeyVal(zaddOpts *schema.ZAddOptions, skipPersistenceCheck bool) (k, v []byte, err error) {
	txn := t.db.NewTransactionAt(math.MaxUint64, true)
	defer txn.Discard()
